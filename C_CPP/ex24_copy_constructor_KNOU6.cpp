...
int main() {
	...
	// 이동생성자
	// add함수를 이용해 v1,v3의 요소들을 더한 결과물 tmp은 v4객체에 복사가 될텐데,
	// tmp는 복사 후에 어차피 삭제(메모리반납)될 것이기 때문에 
	// 이 경우 복사해서 tmp삭제하는 것보다 결과를 그대로 v4로 이동시키면 더 효율적.
	// rvalue 참조를 이용한 이동생성자로 효율개선 가능
	VecF v4(v1.add(v3));
